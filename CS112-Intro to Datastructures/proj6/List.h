...
/*This is the sort method by Joshua Maguire
 * It sorts a list of numbers, ascending order.
 * Preconditions: none, method can be called on an empty list or list of size 1, it just doesn't technically sort it.
 * Postconditions:If the list size was 2 or more, the list was sorted. If duplicate numbers, they end up next to eachother.
If size was < 2, doesn't do anything.
 */
template <class Item>
void List<Item>::sort() {
	if(mySize<2){return;}
	unsigned listCount = 1;//counts sublist(already sorted) size
	Node* prePtr = myFirst;//stays stationary
	Node* thisPtr =  prePtr->myNext;//points at node that is being sorted
	for(unsigned i = 1;i<mySize;i++)//for whole list
	{
		if(prePtr->myItem > thisPtr->myItem)//if next item needs sorting,
		{
			prePtr->myNext=thisPtr->myNext;//free thisPtr...
			if(myFirst->myItem > thisPtr->myItem)//if its lowest number
			{
				thisPtr->myNext=myFirst;
				myFirst=thisPtr;
				thisPtr=prePtr->myNext;//reset thisPtr
			}
			else//else, it needs to be compared to the already sorted items (listCount is how many have been sorted)
			{
				Node* peakPtr = myFirst;
				for(unsigned ii=0;ii<listCount;ii++)//find where it should go
				{
					if(thisPtr->myItem < peakPtr->myNext->myItem)
					{
						thisPtr->myNext=peakPtr->myNext;
						peakPtr->myNext=thisPtr;
						thisPtr=prePtr->myNext;//reset thisPtr
						break;//stop sorting
					}
					peakPtr=peakPtr->myNext;
				}
			}
			listCount++;
		}//next item was sorted.
		else//next item didn't need sorting, update pointers.
		{
			prePtr=prePtr->myNext;
