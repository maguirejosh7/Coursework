...
void VecTester::testWriteSubscipt() const {
	cout << "Testing []... " << flush;
		// empty case
		Vec v0;
		try {
			v0[0] = 11;
			cerr << "operator[] succeeded on empty Vec";
			exit(1);
		} catch (range_error) {
			cout << " 0 " << flush;
		}
...
void VecTester::testReadSubscript() const {
	cout << "Testing []..." << flush;
	// empty Vec
	Vec v0;
	try {
	...
	try {
		v2[3];
		cerr << "operator[] succeeded beyond end of Vec";
		exit(1);
	} catch (range_error) {
		cout << " 2 " << flush;
	}
