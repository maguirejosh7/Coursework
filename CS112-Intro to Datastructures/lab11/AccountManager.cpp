...
void AccountManager::changeUserName(const string& oldUserName, const string& newUserName) {
	...
	if(it!=myAccounts.end())
	{
		...
		try{
			...
			createNewAccount(newUserName,pw);
		}catch(Exception& ex){
			throw Exception("AccountManager::changeUserName","error deleting/creating account!");
		}
	}
	else{
		throw Exception("AccountManager::changeUserName","account not found!");
	}
}
...
void AccountManager::deleteAccount(const string& userName) {
	...
	if(it != myAccounts.end()){myAccounts.erase(it);}
	else
	{
		throw Exception("AccountManager::deleteAccount","account doesn't exist!");
	}
...
void AccountManager::changePassword() {
	...
	if (newPassword == newPasswordCheck) {
		...
		cout << "The two passwords do not match.\n" << endl;
	}
}
...
void AccountManager::changePassword(const string& userName,
	...
	if(it!=myAccounts.end()){
		try{
			...
			throw Exception("AccountManager::changePassword","error deleting/creating account!");
		}
	}
	else{
		throw Exception("AccountManager::changePassword","account not found!");
	}
}
